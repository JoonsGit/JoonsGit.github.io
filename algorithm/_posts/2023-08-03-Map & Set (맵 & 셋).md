...
> 위의 비교에서 무엇을 알 수 있는가? 삽입시에는 연속 컨테이너보다 긴 시간이 걸리지만, **탐색시에는 연속 커네이너에 비해 빠르게 탐색할 수 있다는 장점**이 있다는 것이다. 미미한 차이라고 느낄 수도 있지만 자료의 크기가 커지고 양이 방대해질 수록 와닿는 시간 차이가 지대해질 것 이다. 연속 컨테이너는 항상 정렬된 상태를 유지하려하기 때문에 이러한 차이가 발생하는 것이라고 생각하면 되겠다. 
