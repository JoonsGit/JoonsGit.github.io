...
> template로 사용하는 변수가 2개 이상일 경우에는 함수이름 뒤에 <> 안에 명확하게 사용하지 않는다. <br>
>
...
>왜 사용해야되는걸까? <br>
>
>일종의 안전장치**라고 생각하면 좋다. 우리가 프로그램을 작성하는 도중에 실수로 상수화 시키고 싶었던 변수의 값을 건드려도 알 수 없다. <br>
>
...
>예외처리를 if문으로 해도 되는데 왜 굳이 try catch 를 쓰는 것일까? if문으로 처리해도 상관은 없다. 다만 해당 코드가 **예외처리를 위한 파트임을 보다 명시적으로 나타내고자** try catch문을 사용한다고 생각하면 편하겠다.
...
>이를 보완하기 위한 것이 inline 함수인데, inline 함수는 일반 함수와 달리 **함수 내부의 코드를 함수를 호출한 시점에 그대로 대입시켜 넣는다. 때문에 실행 위치가 왔다갔다 하며 시간을 허비할 필요 없이 다수의 함수 호출을 보다 빠르게 처리**할 수 있게 되는 것이다.
